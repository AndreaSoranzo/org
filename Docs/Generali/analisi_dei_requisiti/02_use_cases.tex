...
\SubUseCase{Inserimento di un valore}
\UCdsc
{ % ATTORE
    \begin{itemize}
        \item Utente in ambiente 3D.
...
{ % DESCRIZIONE
    \begin{itemize}
        \item Questo caso d'uso consente all'utente di inserire un valore numerico positivo che sarà utilizzato per applicare un filtro sui dati visualizzati nel grafico 3D. 
        L'utente può inserire un valore per determinare il numero di barre da visualizzare, ad esempio per filtrare gli N valori più alti o più bassi del grafico, in base al tipo di filtro selezionato.
    \end{itemize}
}
{ % PRECONDIZIONI
    \begin{itemize}
